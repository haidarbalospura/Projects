...
turnX.addEventListener('click', () => { //this selects the X button
  turn = 'X'
  chooseTurn.style.display = 'none'
})
turnO.addEventListener('click', () => {//this selects the O button
  turn = 'O'
  chooseTurn.style.display = 'none'
...
singleplayer.addEventListener('click', () => {//for single player
...
doubleplayer.addEventListener('click', () => {//for double player
...
Array.from(boxes).forEach((element) => {
  ...
  element.addEventListener('click', () => {
    if (boxtext.innerText === '' && isgameover === false) {
      ...
      if (!isgameover) {
        document.querySelector('.info').innerText = singlePlayerMode ? "Turn for " + turn : "Turn for " + turn;
        if (singlePlayerMode) {
          setTimeout(() => {
            computerTurn();
            checkWin();
...
const checkWin = () => {
  ...
  wins.forEach(e => {
    if ((boxtext[e[0]].innerText === boxtext[e[1]].innerText) && (boxtext[e[2]].innerText === boxtext[e[1]].innerText) && (boxtext[e[0]].innerText !== '')) {
      ...
      if (e.length === 10) {
        if (screen.width < 951) {//this is the media query for making line
        ...
        if (screen.width < 950) {
          if (e[8] === 90) { //this checks if the line angle is 90 or not
            document.querySelector('.vertical-line').style.height = '50vw'
            document.querySelector('.vertical-line').style.transform = `translate(${e[12]}vw,${e[13]}vw) rotate(${e[14]}deg)`
          }
          else {
          ...
          if (e[8] === 90) {
            ...
            document.querySelector('.vertical-line').style.transform = `translate(${e[9]}vw,${e[10]}vw) rotate(${e[11]}deg)`
...
function computerTurn() {
  let emptyBoxes = Array.from(boxes).filter((element) => element.querySelector('.boxtext').innerText === '');
